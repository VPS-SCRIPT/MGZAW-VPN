...
let rawDbStructure = null; // keeps original shape (array vs { premium_user: [...] })
...
function setDbUrl(url) {
  if (url && url.trim().startsWith('http')) {
    localStorage.setItem('mzz_rtdb_url', url.trim());
  } else {
...
function updateDbStatusUI() {
  ...
  const ping = document.getElementById('dbStatusPing');
...
async function loadUsers() {
  ...
  try {
    ...
    rawDbStructure = data;
    allUsers = parseUsersData(data);
    
    // Sort by Expiration date descending (newest/longest valid first)
    allUsers.sort((a, b) => {
    ...
    updateMetrics();
    showToast("VIP users updated from database", "success");
  } catch (err) {
...
function parseUsersData(data) {
  ...
  if (Array.isArray(data)) {
    ...
    if (Array.isArray(data.premium_user)) {
      list = data.premium_user.filter(item => item !== null);
    } else if (Array.isArray(data.premium_users)) {
      list = data.premium_users.filter(item => item !== null);
    } else if (Array.isArray(data.vip_users)) {
      list = data.vip_users.filter(item => item !== null);
    } else {
...
// ==========================================
// 3. DATE & EXPIRY HELPERS
// ==========================================

// Parse DD/MM/YYYY into JavaScript Date
...
function renderUsersList() {
  ...
  userTableBody.innerHTML = filteredUsers.map((user, idx) => {
  ...
  // Render Mobile Cards
  userCardsMobile.innerHTML = filteredUsers.map((user, idx) => {
    const expired = isUserExpired(user);
    const daysLeft = getDaysRemaining(user);
    const globalIdx = allUsers.indexOf(user);
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    let updatedList = [...allUsers];

    if (editingIndex >= 0) {
      // Preserve any custom fields from _original
      const original = allUsers[editingIndex]._original || {};
      updatedList[editingIndex] = { ...original, ...updatedUserObj };
    } else {
      // Check for duplicate key
      const exists = allUsers.some(u => u.Key.toLowerCase() === key.toLowerCase());
      if (exists) {
        if (!confirm(`Warning: A user with Token Key "${key}" already exists. Do you still want to proceed?`)) {
          saveBtn.disabled = false;
          saveBtn.classList.remove('opacity-70');
          return;
        }
      }
      updatedList.unshift(updatedUserObj);
    }
...
async function quickRenew(globalIdx) {
  ...
  // If user hasn't expired yet, add 30 days to existing expiration
...
async function deleteUser(globalIdx) {
  ...
  try {
    let updatedList = allUsers.filter((_, idx) => idx !== globalIdx);
    await saveToFirebase(updatedList);
...
async function saveToFirebase(userList) {
  ...
  // Check if original root was wrapped in { "premium_user": [...] }
  let bodyData = payload;
  if (rawDbStructure && !Array.isArray(rawDbStructure) && typeof rawDbStructure === 'object') {
    if ('premium_user' in rawDbStructure) {
      bodyData = { ...rawDbStructure, premium_user: payload };
    }
  }
